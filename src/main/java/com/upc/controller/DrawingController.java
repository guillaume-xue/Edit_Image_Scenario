...
public class DrawingController implements MouseListener, MouseMotionListener {
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        switch (selectedTool) {
            case 0: // Stylo
                view.drawLine(ox, oy, x, y, selectedColor);
                ox = x;
                oy = y;
                break;
            case 1: // Gomme
                view.drawLine(ox, oy, x, y, view.getBackground()); // Utiliser la couleur de fond pour effacer
                ox = x;
                oy = y;
                break;
            case 2: // Cercle
                ...
                view.setPreviewShape("Cercle", startX, startY, width, height);
                break;
            case 3: // Carré
                int side = Math.max(Math.abs(x - ox), Math.abs(y - oy));
                int squareX = Math.min(ox, x);
                int squareY = Math.min(oy, y);
                view.setPreviewShape("Carré", squareX, squareY, side, side);
                break;
            default:
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        switch (selectedTool) {
            case 2: // Cercle
                ...
                int startX = Math.min(ox, x);
                int startY = Math.min(oy, y);
                view.drawOval(startX, startY, width, height, selectedColor);
                break;
            case 3: // Carré
                int side = Math.max(Math.abs(x - ox), Math.abs(y - oy));
                int squareX = Math.min(ox, x);
                int squareY = Math.min(oy, y);
                view.drawRect(squareX, squareY, side, side, selectedColor);
                break;
        }
